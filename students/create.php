...
// --- Generate student_id ---
// Format: {current_year}-{4-digit total count of all students + 1}
// e.g. 2026-0001, 2026-0002 ...
$year = date('Y');
$cntStmt = $pdo->query("SELECT COUNT(*) FROM students");
$cnt     = (int) $cntStmt->fetchColumn();
$student_id_preview = $year . '-' . str_pad($cnt + 1, 4, '0', STR_PAD_LEFT);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $cntStmt = $pdo->query("SELECT COUNT(*) FROM students");
        $cnt     = (int) $cntStmt->fetchColumn();
        $student_id = date('Y') . '-' . str_pad($cnt + 1, 4, '0', STR_PAD_LEFT);
